...
static uint64_t polyglot_rands[] = {
...
uint64_t zobrist_hash(struct position *pos) {
    ...
    for (int square = 0; square < 64; square++) {
        if (pos->board[square] != NO_PIECE) {
			...
			hash ^= polyglot_rands[64 * (type * 2 + (1 - color)) + square];
        }
    }

    // XOR castling rights
	if (pos->castling_rights[WHITE] & KING_SIDE) {
		hash ^= polyglot_rands[768];
	}
	if (pos->castling_rights[WHITE] & QUEEN_SIDE) {
		hash ^= polyglot_rands[769];
	}
	if (pos->castling_rights[BLACK] & KING_SIDE) {
		hash ^= polyglot_rands[770];
	}
	if (pos->castling_rights[BLACK] & QUEEN_SIDE) {
		hash ^= polyglot_rands[771];
	}

	// XOR en passant square
	if (pos->en_passant_square != NO_SQUARE) {
        int file = FILE(pos->en_passant_square);
        hash ^= polyglot_rands[772 + file];
	}

	// XOR side to move
	if (pos->side_to_move == WHITE) {
		hash ^= polyglot_rands[780];
	}
